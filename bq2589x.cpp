...
bq2589x_vbus_type bq2589x::get_vbus_type() {
  ...
  if (ret) return (BQ2589X_VBUS_UNKNOWN);
...
int bq2589x::set_otg_volt(uint16_t volt) {
  ...
  if (volt < BQ2589X_BOOSTV_BASE) volt = BQ2589X_BOOSTV_BASE;
  if (volt >
      BQ2589X_BOOSTV_BASE +
...
int bq2589x::set_otg_current(int curr) {
  ...
  return update_bits(BQ2589X_REG_0A, BQ2589X_BOOST_LIM_MASK,
...
int bq2589x::adc_start(bool oneshot) {
  ...
  if (oneshot)
    ret = update_bits(BQ2589X_REG_02, BQ2589X_CONV_START_MASK,
                      BQ2589X_CONV_START << BQ2589X_CONV_START_SHIFT);
  else
    ret = update_bits(BQ2589X_REG_02, BQ2589X_CONV_RATE_MASK,
...
int bq2589x::adc_stop() {
  return update_bits(BQ2589X_REG_02, BQ2589X_CONV_RATE_MASK,
...
int bq2589x::adc_read_battery_volt() {
  ...
  if (ret) {
    ...
    if ((val & BQ2589X_BATV_MASK) == 0) return 0;
...
int bq2589x::adc_read_sys_volt() {
  ...
  if (ret) {
    ...
    if ((val & BQ2589X_SYSV_MASK) == 0) return 0;
...
int bq2589x::set_charge_current_limit(int curr) {
  ...
  return update_bits(BQ2589X_REG_04, BQ2589X_ICHG_MASK,
...
int bq2589x::set_term_current(int curr) {
  ...
  return update_bits(BQ2589X_REG_05, BQ2589X_ITERM_MASK,
...
int bq2589x::set_prechg_current(int curr) {
  ...
  return update_bits(BQ2589X_REG_05, BQ2589X_IPRECHG_MASK,
...
int bq2589x::set_chargevoltage(int volt) {
  ...
  return update_bits(BQ2589X_REG_06, BQ2589X_VREG_MASK,
...
int bq2589x::set_input_volt_limit(int volt) {
  ...
  return update_bits(BQ2589X_REG_0D, BQ2589X_VINDPM_MASK,
...
int bq2589x::set_input_current_limit(int curr) {
  ...
  return update_bits(BQ2589X_REG_00, BQ2589X_IINLIM_MASK,
...
int bq2589x::set_vindpm_offset(int offset) {
  ...
  return update_bits(BQ2589X_REG_01, BQ2589X_VINDPMOS_MASK,
...
int bq2589x::set_watchdog_timer(uint8_t timeout) {
  return update_bits(BQ2589X_REG_07, BQ2589X_WDT_MASK,
                     (uint8_t)((timeout - BQ2589X_WDT_BASE) / BQ2589X_WDT_LSB)
                         << BQ2589X_WDT_SHIFT);
}
...
uint8_t bq2589x::get_fault() {
  uint8_t val;
  if (read_byte(&val, BQ2589X_REG_0C) == 0) return val;
  return BQ2589X_ERR;
}
...
int bq2589x::set_sys_min(int volt) {
  ...
  val = (volt - BQ2589X_SYS_MINV_BASE + BQ2589X_SYS_MINV_LSB / 2) /
        BQ2589X_SYS_MINV_LSB;
  return update_bits(BQ2589X_REG_03, BQ2589X_SYS_MINV_MASK,
                     val << BQ2589X_SYS_MINV_SHIFT);
}
...
int bq2589x::set_rechg_volt_offset(int mvolt) {
  if (mvolt < 100 || mvolt > 200) return BQ2589X_ERR;

  uint8_t val = (mvolt < 150) ? BQ2589X_VRECHG_100MV : BQ2589X_VRECHG_200MV;
  return update_bits(BQ2589X_REG_06, BQ2589X_VRECHG_MASK,
